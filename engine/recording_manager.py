...
def _get_ffmpeg_encoders():
    ...
    try:
        ...
        _FFMPEG_ENCODERS = result.stdout if result.stdout else ""
    except Exception:
        _FFMPEG_ENCODERS = ""
...
class RecordingManager:
    ...
    def handle_recording(self, frame, motion_detected, last_motion_time, stop_recording_cb, trigger_source=None, ai_results=None):
        ...
        max_len = self.config.get('max_video_length', 0)
        if self.is_recording and max_len > 0:
            if time.time() - self.recording_start_time > max_len:
                logger.info(f"Camera {self.camera_name} (ID: {self.camera_id}): Max video length reached, splitting file")
                stop_recording_cb()
    ...
    def start_recording(self, width, height, pre_buffer_frames, event_callback=None, reason="Manual", trigger_source=None):
        ...
        format_str = self.config.get('video_file_name', '%Y-%m-%d/%H-%M-%S').replace('%q', '00')
        timestamp_path = datetime.now().strftime(format_str)
        output_dir = f"{WATCHX_STORAGE_ROOT}/{self.camera_id}"
        ...
        if self.passthrough_error_count > 1:
            ...
            self.passthrough_active = self.config.get('video_passthrough', False)
        ...
        quality = self.config.get('video_quality', 75)
        ...
        if hw_accel_enabled:
            if hw_accel_type in ['vaapi', 'intel', 'amd', 'auto'] and os.path.exists('/dev/dri') and _has_ffmpeg_encoder('h264_vaapi'):
                video_codec = 'h264_vaapi'
                codec_specific_args = ['-vaapi_device', '/dev/dri/renderD128', '-vf', 'format=nv12,hwupload', '-qp', str(int(crf * 0.7))]
            elif hw_accel_type == 'nvidia' and _has_ffmpeg_encoder('h264_nvenc'):
                video_codec = 'h264_nvenc'
                codec_specific_args = ['-preset', 'fast', '-cq:v', str(crf)]
            elif hw_accel_type == 'nvidia':
                logger.warning(f"Camera {self.camera_name} (ID: {self.camera_id}): NVIDIA encode requested but h264_nvenc is unavailable. Falling back to libx264.")
