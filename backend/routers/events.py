        ...
        try:
            ...
            if should_notify_tg and tg_token and tg_chat:
                try:
                    ...
                    if event_type == "event_start":
                        ...
                        prefix = "🤖 <b>AI</b> " if "AI Engine" in source else "🚨 "
            ...
            if should_notify_email and smtp_server and email_recipient:
                try:
                    msg = MIMEMultipart()
                    msg['Subject'] = subject
                    msg['From'] = smtp_from or "watchx@localhost"
...
def delete_event_files(event: models.Event) -> int:
    ...
    for ptype, raw_path in paths:
        path = data_path_from_engine_path(raw_path)
...
def cleanup_orphaned_file(file_path: str, camera_id: int):
    ...
    local_path = data_path_from_engine_path(file_path)
...
async def webhook_event(
    ...
    event_type = payload.get("type") # event_start, picture_save, video_end
    ...
    elif event_type == "video_end" or event_type == "picture_save":
...
def process_webhook_file_event(camera_id: int, event_type: str, payload: dict, in_schedule: bool):
    ...
    try:
        ...
        event_data = schemas.EventCreate(
            camera_id=camera_id,
            timestamp_start=ts,
            type="video" if event_type == "video_end" else "snapshot",
            event_type="motion",
            file_path=file_path,
            file_size=file_size,
        ...
        if event_type == "video_end":
            # Remove from active cameras on video end
...
async def download_event(event_id: int, request: Request, token: Optional[str] = None):
    ...
    file_path = data_path_from_engine_path(file_path)
