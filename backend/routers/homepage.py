...
def get_homepage_stats(
    ...
    # WatchX usage
    watchx_usage = db.query(func.sum(models.Event.file_size)).scalar() or 0
    
    storage_used_gb = round(watchx_usage / (1024**3), 2)
    storage_total_gb = round(total / (1024**3), 1)
    storage_percent = round((watchx_usage / total) * 100) if total > 0 else 0
