...
def perform_group_action(group_id: int, action: schemas.GroupAction, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth_service.get_current_active_admin)):
    ...
    if action.action == "enable_motion":
        ...
        # Category mapping
        category_map = {
            'recording': ['recording_mode', 'video_quality', 'video_passthrough', 'max_video_length', 'preserve_videos', 'max_storage_gb', 'live_view_mode', 'rtsp_transport', 'sub_rtsp_transport', 'record_audio'],
    ...
    if modified_count > 0:
        db.commit()
        # Sync updated cameras to the WatchX engine
        for cam in group.cameras:
