...
def run_backup(is_manual: bool = False):
    ...
    try:
        ...
        with database.get_db_ctx() as db:
            ...
            filename = f"watchx_backup_{prefix}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def get_last_auto_backup_timestamp():
    ...
    try:
        ...
        backups = [f for f in os.listdir(BACKUP_DIR) if (f.startswith("watchx_backup_auto_") or f.startswith("watchx_auto_backup_")) and f.endswith(".json")]
        if not backups:
            return 0
        ...
        # Files are named watchx_backup_auto_YYYYMMDD_HHMMSS.json
...
def cleanup_old_backups(retention: int):
    ...
    try:
        ...
        # Support both new watchx_backup_auto_ and legacy watchx_auto_backup_
        backups = [f for f in os.listdir(BACKUP_DIR) if (f.startswith("watchx_backup_auto_") or f.startswith("watchx_auto_backup_")) and f.endswith(".json")]
        backups.sort() 
