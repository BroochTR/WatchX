...
def get_cameras_for_user(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Camera).join(models.Camera.clients).filter(models.User.id == user_id).offset(skip).limit(limit).all()

def user_can_access_camera(db: Session, user_id: int, camera_id: int) -> bool:
    return db.query(models.Camera.id).join(models.Camera.clients).filter(
...
def create_camera(db: Session, camera: schemas.CameraCreate):
    ...
    client_user_ids = create_data.pop("client_user_ids", None)
    ...
    if client_user_ids is not None:
        clients = db.query(models.User).filter(
            models.User.id.in_(client_user_ids),
            models.User.role == "client"
        ).all()
        db_camera.clients = clients
    db.add(db_camera)
    db.commit()
...
def update_camera(db: Session, camera_id: int, camera: schemas.CameraCreate):
    ...
    if db_camera:
        ...
        client_user_ids = update_data.pop("client_user_ids", None)
        # Convert list to JSON string for the DB column only if it's a list
