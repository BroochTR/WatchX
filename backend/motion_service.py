...
from utils import WATCHX_STORAGE_ROOT

import logging

# WatchX Engine Control URL
...
def camera_to_config(cam: Camera, opt_settings: dict = None) -> dict:
    """ Convert DB Camera model to WatchX Engine config dict """
    ...
    config = {
        ...
        "video_quality": cam.video_quality or 75,
        "video_passthrough": cam.video_passthrough if cam.video_passthrough is not None else False,
        "max_video_length": cam.max_video_length or 0,
        "record_audio": cam.record_audio if cam.record_audio is not None else False,
        "video_file_name": cam.video_file_name or "%Y-%m-%d/%H-%M-%S",
        ...
        "storage_path": WATCHX_STORAGE_ROOT,
        # AI & Tracking
...
def generate_motion_config(db: Session):
    ...
    while True:
        try:
            ...
            retry_count += 1
            logger.info(f"Waiting for WatchX Engine... (Attempt {retry_count})")
            time.sleep(5)
...
def sync_global_config(db: Session):
    """Sync global optimization settings to the WatchX Engine config endpoint"""
    opt_settings = get_optimization_settings(db)
    ...
    try:
        ...
        if resp.status_code == 200:
            logger.info("Successfully synced global config to WatchX Engine")
            return True
        else:
...
def trigger_snapshot(camera_id: int):
    """Trigger a snapshot in WatchX Engine"""
    try:
...
def stop_camera(camera_id: int):
    """Stop a camera in WatchX Engine"""
    try:
