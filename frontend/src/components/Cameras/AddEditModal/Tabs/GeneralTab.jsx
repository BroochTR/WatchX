...
export const GeneralTab = ({ newCamera, setNewCamera, clientUsers = [] }) => {
    const selectedClientIds = Array.isArray(newCamera.client_user_ids) ? newCamera.client_user_ids : [];
    const toggleClient = (clientId) => {
        const next = selectedClientIds.includes(clientId)
            ? selectedClientIds.filter(id => id !== clientId)
            : [...selectedClientIds, clientId];
        setNewCamera({ ...newCamera, client_user_ids: next });
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {clientUsers && (
                <>
                    <SectionHeader title="Client Access" description="Choose which clients can view this camera" />
                    {clientUsers.length === 0 ? (
                        <div className="text-xs text-muted-foreground bg-muted/30 border border-border rounded-lg p-3">
                            No client users available. Create a user with role "client" first.
                        </div>
                    ) : (
                            ...
                            {clientUsers.map((client) => {
                                ...
                                return (
                                    <label key={client.id} className="flex items-center gap-2 text-sm cursor-pointer">
                                        ...
                                        />
                                        <span className="text-foreground">{client.username}</span>
                                    </label>
                                );
                            })}
                        </div>
