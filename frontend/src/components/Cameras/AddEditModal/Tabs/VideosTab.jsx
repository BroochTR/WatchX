...
export const VideosTab = ({ editingId, newCamera, setNewCamera, stats, handleCleanup, globalSettings }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className={`p-3 rounded-lg text-xs mb-4 transition-all duration-300 ${
                ...
                <Toggle
                    label="Passthrough Recording"
                    checked={hasPrivacyMasks ? false : !!newCamera.video_passthrough}
                    onChange={(val) => setNewCamera({ ...newCamera, video_passthrough: val })}
            ...
            <Slider
                label="Video Quality"
                value={newCamera.video_quality}
                onChange={(val) => setNewCamera({ ...newCamera, video_quality: val })}
                min={10}
                max={100}
                step={5}
                unit="%"
            ...
            <Slider
                ...
                value={newCamera.max_video_length || 120}
                onChange={(val) => setNewCamera({ ...newCamera, max_video_length: val })}
                min={60}
                max={300}
                step={30}
                unit=" sec"
            ...
            <InputField
                ...
                value={newCamera.video_file_name}
                onChange={(val) => setNewCamera({ ...newCamera, video_file_name: val })}
                placeholder="%Y-%m-%d/%H-%M-%S"
            />
            <SelectField
                label="Preserve Videos"
                value={newCamera.preserve_videos}
                onChange={(val) => setNewCamera({ ...newCamera, preserve_videos: val })}
                options={[
                ...
                <div className="mb-6 p-4 bg-muted/30 rounded-lg border border-border">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="font-semibold text-sm">Video Storage</span>
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                        <div>
                            ...
                            <p className="text-lg font-bold">{stats.details.cameras[editingId].videos.size_gb} GB</p>
                        </div>
                        <div>
                            <p className="text-xs text-muted-foreground">Total Files</p>
                            <p className="text-lg font-bold">{stats.details.cameras[editingId].videos.count}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                            <span className="text-muted-foreground">
                                {newCamera.max_storage_gb > 0
                                    ? `${Math.round((stats.details.cameras[editingId].videos.size_gb / newCamera.max_storage_gb) * 100)}% Used`
                                    : 'Unlimited Storage'}
                            </span>
                            <span className="text-muted-foreground">
                        ...
                        <div className="h-2 w-full bg-secondary/50 rounded-full overflow-hidden">
                            <div
                                className={`h-full ${newCamera.max_storage_gb > 0 && stats.details.cameras[editingId].videos.size_gb > newCamera.max_storage_gb ? 'bg-red-500' : 'bg-blue-500'}`}
                                style={{
                                    width: newCamera.max_storage_gb > 0
                                        ? `${Math.min((stats.details.cameras[editingId].videos.size_gb / newCamera.max_storage_gb) * 100, 100)}%`
                                        : '0%'
                                }}
                            />
                        </div>
