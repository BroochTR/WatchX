...
import { Activity, Camera, HardDrive, ShieldAlert, Film, Image, CalendarClock, Cpu, MemoryStick, Settings, GripVertical, GripHorizontal, Network, Database, Zap, Bot, Brain } from 'lucide-react';
...
export const Dashboard = () => {
    ...
    // Widget Order State
    const DEFAULT_ORDER = [
        'storage_videos', 'storage_pictures', 'storage_retention',
    ...
    const WIDGET_REGISTRY = {
                ...
                <div className="bg-card border border-border rounded-xl p-6 h-full">
                    ...
                    <div className="space-y-4">
                        <div className="p-3 rounded-lg bg-muted/30">
                            ...
                            <p className="text-xl font-bold">{stats.details?.global.videos.count || 0}</p>
                        </div>
                        <div className="p-3 rounded-lg bg-muted/30">
                            <p className="text-xs text-muted-foreground">Disk Usage</p>
                            <p className="text-xl font-bold">{stats.details?.global.videos.size_gb || 0} GB</p>
                        </div>
                    </div>
                </div>
            )
        ...
        ai_processor: {
            ...
            render: () => {
                ...
                const isNvidia = ai?.hardware === 'nvidia';
                const isInitialized = ai?.initialized;
                const showNvidiaHint = !isNvidia && ai?.nvidia_reason && (ai?.requested_hardware === 'nvidia' || (ai?.requested_hardware === 'auto' && ai?.model_type === 'yolo_v8'));
                return (
                    <div className="p-4 md:p-6 rounded-xl bg-card border border-border hover:shadow-lg transition-shadow duration-300 group h-full relative flex flex-col justify-between">
                        <div className="flex items-center justify-between mb-4">
                        ...
                        <div>
                            ...
                            {showNvidiaHint && (
                                <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed break-words">
                                    {ai.nvidia_reason}
                                </p>
                            )}
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1 pr-2">
                        {recentEvents.length === 0 ? (
                            ...
                            recentEvents.map((evt) => {
                                ...
                                try {
                                    if (evt.ai_metadata) {
                                        // WatchX stores labels as a comma-separated string
