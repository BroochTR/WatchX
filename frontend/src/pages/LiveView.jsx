...
import { Camera, CameraOff, Maximize2, Minimize2, Settings, Image as ImageIcon, Play, Square, Power, Disc, Grid, X, Volume2, VolumeX } from 'lucide-react';
    ...
    return (
        <div ref={containerRef} className={`video-container relative w-full bg-black rounded-xl overflow-hidden ${isFullscreen ? 'h-full' : 'aspect-video'} group transition-all duration-300 ${isFocused ? 'ring-4 ring-primary z-30' : 'z-10'}`}>
            ...
            {isUnauthorized ? (
                ...
                <img src="/no-signal.svg" alt="No Signal" className="absolute inset-0 w-full h-full object-cover" />
            ) : (
                <>
...
export const LiveView = () => {
    ...
    useEffect(() => {
        if (user?.role !== 'client' || !isGroupView) {
            return;
        }

        setIsGroupView(false);
        localStorage.setItem('liveViewGroupBy', 'false');
    }, [user?.role, isGroupView]);
    ...
    const gridColumnsClass = focusCameraId ? 'grid-cols-1' : (
        columnSetting === 'auto' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' :
        ...
        <div className="h-full flex flex-col px-5 py-4 lg:p-8">
            <div className="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                    ...
                    <div className="flex items-center gap-2">
                        {/* Group Selector */}
                        {user?.role !== 'client' && availableGroups.length > 0 && (
            ...
            {/* Group View Render Logic */}
            {
                isGroupView && selectedGroup === 'all' && !focusCameraId ? (
                                ...
                                <>
                                    {sortedGroupNames.map(groupName => (
                                        <div key={groupName}>
                                            ...
                                            <div className={`grid gap-2 sm:gap-4 ${gridColumnsClass}`}>
                    ...
                    <div className={`grid gap-2 sm:gap-4 flex-1 min-h-0 pr-2 ${gridColumnsClass}`}>
