...
export const Settings = () => {
    ...
    const [newUser, setNewUser] = useState({ username: '', password: '', role: 'client', email: '' });
    ...
    const normalizeAiHardware = useCallback((value) => {
        if (value === 'tpu') {
            return 'cpu';
        }
        return value || 'auto';
        ...
        try {
            ...
            if (res.ok) {
                ...
                setGlobalSettings(prev => ({
                    ...
                    ai_model: data.ai_model?.value === 'yolo_v8' ? 'yolo_v8' : prev.ai_model,
                    ai_hardware: normalizeAiHardware(data.ai_hardware?.value || prev.ai_hardware),
        ...
        try {
            await fetch('/api/settings/bulk', {
                ...
                body: JSON.stringify({
                    ...
                    ai_model: 'yolo_v8',
                    ai_hardware: normalizeAiHardware(globalSettings.ai_hardware),
    ...
    const handleCreateUser = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                setNewUser({ username: '', password: '', role: 'client', email: '' });
    ...
    const openPasswordModal = (targetUser = null) => {
        const normalizedTargetUser = targetUser && targetUser.id !== user.id ? targetUser : null;
        setPwdTargetUser(normalizedTargetUser);
    ...
    return (
        <div className="space-y-12 relative w-full pb-52 min-w-0 max-w-full overflow-hidden">
            ...
            {user?.role === 'admin' && (
                <div className="fixed bottom-6 inset-x-5 sm:inset-x-auto sm:right-8 z-[60] flex justify-center sm:justify-end pointer-events-none">
                    <button
                        onClick={handleSave}
